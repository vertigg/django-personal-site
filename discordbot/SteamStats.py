...
handler = logging.FileHandler(
    filename='logs/steamstats.log', encoding='utf-8', mode='w')
...
parser.add_argument('-i', '--fix-icons',
                    help='only fixing icons', action='store_true')
parser.add_argument('-d', '--decorate',
                    help='decorate google spreadsheet for first use', action='store_true')
# Some games doesn't actually have descriptions
parser.add_argument(
    '-n', '--nodesc', help='only fixing icons', action='store_true')
parser.add_argument('-g', '--game', help='other game id',
                    action='store', dest='GAMEID', required=False, default='232090')
parser.add_argument('-gs', '--spreadsheet', help='use another spreadsheet',
                    dest='GSNAME', default='Steam Achievements')
parser.add_argument('-ws', '--worksheet', help='use another worksheet',
                    dest='WSNAME', required=False, default='KF2')
...
def createGameSettingsFile():
    ...
    for player in ALLPLAYERS:
        ...
        payload = {'key': STEAM_API_KEY,
                   'appid': args['GAMEID'], 'steamid': current['SteamID']}
...
def getMasterDict():
    ...
    r = requests.get(
        'http://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v0002/?key={0}&appid={1}&l=english&format=json'.format(STEAM_API_KEY, args['GAMEID']))
    data = json.loads(r.text, encoding='utf-8')
    master = data['game']['availableGameStats']['achievements']
    gameName = data['game']['gameName']
    logger.info('Game {d[game][gameName]}, version {d[game][gameVersion]}, total {len} achievements'.format(
        d=data, len=len(master)))  # Some games may fail cause of 'tm' shit
    ...
    for player in gameSettings['users']:
        ...
        if len(listPlayer) < len(master):
            ...
            ws.update_acell('{0}1'.format(stats['Total']), '=SUM({2}1:{2}{0})/({1}*{0})'.format(
                len(gameSettings['users']), len(master), stats['Done']))
        else:
            changes = 0
...
def playerFullColumn(current, achievements, player):
    ...
    logger.info('Column {0} for player {1} changed'.format(
        current['letter'], player))
...
def decorate():
    ...
    ws.update_acell('{0}1'.format(gameSettings['stats']['Total']), '=SUM({2}1:{2}{0})/({1}*{0})'.format(
        len(gameSettings['users']), len(master), gameSettings['stats']['Done']))
    for player in gameSettings['users']:
        current = gameSettings['users'][player]
        ws.update_acell('{0}{1}'.format(
            current['letter'], gameSettings['achOffset']), player)
        ws.update_acell('{0}{1}'.format(
            gameSettings['stats']['Name'], current['statsIndex']), player)
